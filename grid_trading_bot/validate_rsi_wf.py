...
用法:
    python validate_rsi_wf.py --leverage 18
    python validate_rsi_wf.py --leverage 18 --full
"""
...
# ============================================================
# 優化後最佳參數 (run_backtest.py defaults / settings.yaml)
# ============================================================
OPTIMIZED_PARAMS = {
    # v5 Optuna 優化 (2026-02-01) from settings.yaml
    "rsi_period": 5,
...
def run_backtest(klines: list[Kline], leverage: int = 18, params: dict | None = None) -> dict:
    ...
    config = RSIGridStrategyConfig(
        ...
        rsi_block_threshold=p["rsi_block_threshold"],
        atr_period=p["atr_period"],
        grid_count=p["grid_count"],
        atr_multiplier=Decimal(str(p["atr_multiplier"])),
        stop_loss_atr_mult=Decimal(str(p["stop_loss_atr_mult"])),
        take_profit_grids=p["take_profit_grids"],
        max_hold_bars=p["max_hold_bars"],
        use_trailing_stop=p["use_trailing_stop"],
        trailing_activate_pct=p["trailing_activate_pct"],
        trailing_distance_pct=p["trailing_distance_pct"],
        use_volatility_filter=p["use_volatility_filter"],
        vol_atr_baseline_period=p["vol_atr_baseline_period"],
        vol_ratio_low=p["vol_ratio_low"],
        vol_ratio_high=p["vol_ratio_high"],
    )
...
def walk_forward_validation(klines: list[Kline], n_splits: int = 9, leverage: int = 18):
    """Walk-Forward 驗證."""
    print(f"\n{'=' * 60}")
    print(f"  Walk-Forward 驗證 (RSI Grid)")
...
def out_of_sample_test(klines: list[Kline], leverage: int = 18):
    ...
    print(f"  策略: RSI Grid, 槓桿: {leverage}x")
    print(f"{'=' * 60}")
...
def parameter_sensitivity_test(klines: list[Kline], leverage: int = 18):
    ...
    base_params = {
        ...
        "stop_loss_atr_mult": OPTIMIZED_PARAMS["stop_loss_atr_mult"],
        "max_hold_bars": OPTIMIZED_PARAMS["max_hold_bars"],
    }
...
def main():
    parser = argparse.ArgumentParser(description="RSI Grid Walk-Forward 驗證")
    parser.add_argument(
        "--leverage", "-l",
        type=int,
        default=18,
    ...
    # 1. 完整回測
    print(f"\n{'=' * 60}")
    print(f"  完整回測 (RSI Grid, {leverage}x 槓桿)")
    print(f"{'=' * 60}")
