...
class SupertrendIndicator:
    ...
    def __init__(self, atr_period: int = 10, atr_multiplier: Decimal = Decimal("3.0")):
        ...
        # ATR state for Wilder's smoothing
    ...
    def update(self, kline: Kline) -> Optional[SupertrendData]:
        ...
        if self._prev_lower_band is not None and self._prev_upper_band is not None and self._prev_close is not None:
        ...
        if self._prev_trend == 0:
            ...
            # Was bullish - use current adjusted lower_band for trend determination
            trend = 1 if close > lower_band else -1
        else:
            # Was bearish - use current adjusted upper_band for trend determination
            trend = -1 if close < upper_band else 1
    ...
    def _calculate_atr(self) -> Optional[Decimal]:
        ...
        if not hasattr(self, '_prev_atr') or self._prev_atr is None:
