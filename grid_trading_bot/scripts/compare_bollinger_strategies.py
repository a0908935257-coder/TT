#!/usr/bin/env python3
"""
Compare Bollinger backtest results: Original vs Live-like strategy.
...
def run_backtest(klines: list[Kline], config: BollingerStrategyConfig, name: str) -> dict:
...
async def main():
    """Main entry point."""
    print("="*80)
    print("Bollinger Strategy Comparison: Original vs Live-Like")
    print("="*80)
    ...
    original_config = BollingerStrategyConfig(
        mode=BollingerMode.BB_TREND_GRID,
        bb_period=20,
        bb_std=Decimal("2.0"),
        grid_count=10,
        grid_range_pct=Decimal("0.04"),  # 4% range
        take_profit_grids=1,
    ...
    live_like_config = BollingerStrategyConfig(
        mode=BollingerMode.BB_TREND_GRID,
        bb_period=20,
        bb_std=Decimal("2.0"),
        grid_count=10,
