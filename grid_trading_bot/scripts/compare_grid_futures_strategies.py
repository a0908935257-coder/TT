#!/usr/bin/env python3
"""
Compare Grid Futures backtest results: Original vs Live-like strategy.
...
def run_backtest(klines: list[Kline], config: GridFuturesStrategyConfig, name: str) -> dict:
    ...
    engine_config = BacktestConfig(
        ...
        leverage=config.leverage,        # Use strategy leverage
    )
...
async def main():
    """Main entry point."""
    print("="*80)
    print("Grid Futures Strategy Comparison: Original vs Live-Like")
    print("="*80)
    print("\nFetching historical data...")

    # Parameters (matching settings.yaml - Grid Futures uses 1h timeframe)
    symbol = "BTCUSDT"
    interval = "1h"
    days = 730  # 2 years
    ...
    # Strategy 1: Original (no hysteresis, no cooldown) - matching settings.yaml
    original_config = GridFuturesStrategyConfig(
        grid_count=10,
        direction=GridDirection.NEUTRAL,  # NEUTRAL mode (validated)
        leverage=10,                       # 10x leverage (validated)
        atr_period=14,
    ...
    live_like_config = GridFuturesStrategyConfig(
        grid_count=10,
        direction=GridDirection.NEUTRAL,  # NEUTRAL mode (validated)
        leverage=10,                       # 10x leverage (validated)
        atr_period=14,
